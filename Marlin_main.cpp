  ...
  else if(code_seen('M'))
  {
	...
	case 665: // M665 set delta configurations L<diagonal_rod> R<delta_radius> S<segments_per_sec>
		...
		if (code_seen('Z')) {// set the maxZ
            ZMaxPos = code_value();
			SERIAL_ECHOPAIR("Set the Max Z: ",ZMaxPos);
             SERIAL_ECHOLN("");
        }
		...
		recalc_delta_settings(delta_radius, delta_diagonal_rod, ZMaxPos);
		break;
      ...
      case 666: // M666 set delta endstop and geometry adjustment
	   ...
	   if (code_seen('K')) {
		...
		set_delta_constants();
           ...
           if (code_seen('H')) {// maybe this is the right way of doing it?
...
#ifdef DELTA
void recalc_delta_settings(float radius, float diagonal_rod, float ZMaxPos)
{
